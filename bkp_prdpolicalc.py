...
import time
import prd_auxfunctions as aux
import logging
import textwrap  
import fileinput
import numpy as np  
...
def statprint(ratedat,sim_time,filename,sys_time,date): 
...
def  poli_out(polistat_1, polistat_2):
    with open(filename, 'r') as fin:
    
#------------------Main Funcion----------------#
def main(filename, sim_time): 
    ...
    statprint(ratedat,sim_time,filename,sys_time,date)
...
if __name__=='__main__':
    filename = args[1]
    sim_time = float(args[2])
    main(args['INPUT'], sim_time)
